...
class sts(protocol_packet_handler):
    ...
    def changeId(self, sts_id, new_sts_id):
        result, error = self.unLockEprom(sts_id)
        if result != COMM_SUCCESS:
            return result, error
        sts_comm_result, sts_comm_error = self.write1ByteTxRx(sts_id, STS_ID, new_sts_id)
        if sts_comm_result != COMM_SUCCESS:
            print("locking old")
            self.LockEprom(sts_id)
        else:
            print("locking new")
            sts_comm_result, sts_comm_error = self.LockEprom(new_sts_id)
        return sts_comm_result, sts_comm_error
